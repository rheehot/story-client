...
<script>
  ...
  const onToggleMode = () => {
    dispatch('toggleModel', {
      nextMode: mode === 'REGISTER' ? 'LOGIN' : 'REGISTER',
    });
  };

  const onSubmit = () => {
    dispatch('sendAuthEmail', {
      email,
    });
  };
...
<div class="AuthForm">
  <div class="Upper-Wrapper">
    ...
    <section>
      <h4>이메일로 {modeText}</h4>
      {#if registered !== null}
        <AuthEmailSuccess {registered} />
      {:else}
        <AuthEmailForm {mode} {email} on:keyup="{onChangeEmail}" on:submit="{onSubmit}" />
      {/if}
    </section>
    <section>
  ...
  <div class="foot">
    ...
    <div class="link" tabIndex="{7}" on:click="{onToggleMode}">
      {mode === 'LOGIN' ? '회원가입' : '로그인'}
    </div>
  </div>
</div>
